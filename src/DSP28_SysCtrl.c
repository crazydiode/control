...
// Initalize PLL
	SysCtrlRegs.PLLCR = 0xA;    //5倍频，150M
   // Wait for PLL to lock
   for(i= 0; i< 5000; i++){}
       
// HISPCP/LOSPCP prescale register settings, normally it will be set to default values
   SysCtrlRegs.HISPCP.all = 0x0002;   //HCLK 4分频，37.5M. HCLK最高位75M
   SysCtrlRegs.LOSPCP.all = 0x0002;	  //LCLK 4分频，37.5M
// Peripheral clock enables set for the selected peripherals.   
   SysCtrlRegs.PCLKCR.bit.EVAENCLK=1;   //使能EVA的时钟为HCLK
   SysCtrlRegs.PCLKCR.bit.ADCENCLK=1;   //使能ADC的时钟为HCLK
//   SysCtrlRegs.PCLKCR.bit.EVBENCLK=1;
   SysCtrlRegs.PCLKCR.bit.SCIENCLKA=1;   //使能SCI-A时钟
//   SysCtrlRegs.PCLKCR.bit.SCIENCLKB=1;
   SysCtrlRegs.PCLKCR.bit.MCBSPENCLK =1;  //使能McBSP的时钟为LCLK
